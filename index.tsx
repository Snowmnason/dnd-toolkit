...
// Gate bootstrap logs for web Skia init to keep console clean
...
async function initializeWebApp() {
  if (Platform.OS === "web") {
    try {
      blog.debug("bootstrap", "Loading Skia for web before app render...");
      ...
      await LoadSkiaWeb({
        locateFile: (file) => {
          blog.debug("bootstrap", "Locating file:", file);
      ...
      blog.info("bootstrap", "Skia (CanvasKit) loaded successfully");
    } catch (error) {
      blog.error("bootstrap", "Failed to load Skia:", error);
      blog.warn("bootstrap", "App will continue without Skia canvas features");
