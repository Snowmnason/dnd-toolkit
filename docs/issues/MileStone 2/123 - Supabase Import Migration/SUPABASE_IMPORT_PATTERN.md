...
- Approach: Keep direct `supabase` usage inside `lib/database/` (DB abstraction layer). Everywhere else use the lazy wrapper helpers (`isSupabaseConfiguredLazy()` + `getSupabaseClientLazy()`) to avoid loading the Supabase client until it's needed.
...
```ts
  // Use the lazy wrapper from `lib/database/supabase-lazy`
...
## Checklist for migrating a file
 - Ensure non-database files use `getSupabaseClientLazy` and `isSupabaseConfiguredLazy` from `lib/database/supabase-lazy`.
 - Add an `isSupabaseConfiguredLazy()` guard at function entry before any `supabase` usage.
 - Use `getSupabaseClientLazy()` to obtain the client after the guard.
