...
export const AuthStateManager = {
  ...
  async saveUserData(userData: StoredUserData): Promise<void> {
    try {
      ...
      logger.debug('auth', 'User data saved to storage with timestamp:', { id: userData.id, username: userData.username, isAdmin: userData.isAdmin });
    } catch (error) {
  ...
  async setSession(session: any): Promise<void> {
    try {
      ...
      logger.error('auth', 'Error setting session:', error);
    }
  },
  ...
  async isCacheFresh(): Promise<boolean> {
    try {
      ...
      logger.debug('auth', 'Cache freshness check:', {
      ...
      logger.error('auth', 'Error checking cache freshness:', error);
      return false;
    }
  },
