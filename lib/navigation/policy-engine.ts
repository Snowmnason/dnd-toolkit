...
import { NavigationContext, NavigationDecision, NavigationGuardConfig, NavigationPolicyMode } from '@/type-definitions/navigation-decision';
...
export class PolicyEngine {
  ...
  static getPolicyForRoute(
    ...
    if (mode === 'protected_by_default') {
      ...
      // Check if route requires permission specifically
      if (
        AUTH_CONFIG.protectedRoutes.some((protRoute: string) =>
          toRoute.toLowerCase().includes('world'),
        )
      ) {
        return 'require_permission';
      }
    ...
    const isProtected = AUTH_CONFIG.protectedRoutes.some((protRoute: string) =>
      toRoute.toLowerCase().includes(protRoute.toLowerCase()),
    );
    if (!isProtected) {
      return 'allow_all';
    }
    ...
    if (
      AUTH_CONFIG.protectedRoutes.some((protRoute) =>
        toRoute.toLowerCase().includes('world'),
      )
    ) {
      return 'require_permission';
    ...
    switch (verdict) {
      ...
      case 'require_admin':
        ...
        guards.push({
          ...
          priority: 'normal',
          check: async (ctx: NavigationContext): Promise<NavigationDecision> => {
            // TODO: Implement admin verification (check against admin roles/permissions)
