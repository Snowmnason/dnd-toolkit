 ...
 *
 * @param pathA First path (e.g., '/Main/Characters')
 * @param pathB Second path (e.g., '/main/characters')
 * @returns true if paths match (case-insensitive)
 *
 * @example
 * pathEquals('/Main/Characters', '/main/characters'); // true
 */
export function pathEquals(pathA: string, pathB: string): boolean {
  return canonicalizePath(pathA) === canonicalizePath(pathB);
}

/**
 * Check if a path starts with a given prefix (case-insensitive)
 *
 * @param path The full path (e.g., '/main/characters/123')
 * @param prefix The prefix to check (e.g., '/main/characters')
 * @returns true if path starts with prefix (case-insensitive)
 *
 * @example
 * pathStartsWith('/main/characters/123', '/main/characters'); // true
 */
export function pathStartsWith(path: string, prefix: string): boolean {
  return canonicalizePath(path).startsWith(canonicalizePath(prefix));
}
