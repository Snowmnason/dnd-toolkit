...
Client-side code in `lib/feature-flags/server-sync.ts` invokes this function instead of direct queries:
...
When creating in Supabase Dashboard:
...
This function is designed as a natural growth point:
...
- **Audit Logging** — Log flag returned to which users for analytics
...
- Check database RLS policies allow reads (service role key bypasses RLS)
- Verify JWT token is valid and contains `sub` claim

### JWT verification fails

- Ensure Bearer token is in `Authorization: Bearer <token>` format
- Verify token was issued by Supabase (signed with correct key)
- Check token has not expired (check `exp` claim)
