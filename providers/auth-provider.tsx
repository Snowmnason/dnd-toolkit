...
} from "@/lib/database/supabase-lazy";
    ...
    const fetchSession = async () => {
      ...
      try {
        if (!(await isSupabaseConfiguredLazy())) {
          logger
            .category("auth")
            .warn("Supabase not configured for session fetch");
          setIsLoading(false);
          return;
        }
        ...
        if (error) {
          logger
            .category("auth")
            .error("Failed to fetch session", { error: error.message });
        } else {
          logger
            .category("auth")
      ...
      if (session) {
        try {
          if (!(await isSupabaseConfiguredLazy())) {
            logger
              .category("auth")
              .warn("Supabase not configured for profile fetch");
            setIsLoading(false);
            return;
          }
          const supabase = await getSupabaseClientLazy();
