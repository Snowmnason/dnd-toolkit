...
function ensureDist() {
  if (!fs.existsSync(DIST_DIR)) {
    console.warn(
      '[harden-web] dist folder missing; run "expo export -p web" first',
    );
    return false;
  }
...
function extractInlineContents(html) {
  ...
  const scriptContents = scriptMatches
    .map((m) => (m[1] || "").trim())
    .filter(Boolean);
  const styleContents = styleMatches
    .map((m) => (m[1] || "").trim())
    .filter(Boolean);

  const scriptHashes = scriptContents
    .map(computeCspHash)
    .map((hash) => `'${hash}'`);
  const styleHashes = styleContents
    .map(computeCspHash)
    .map((hash) => `'${hash}'`);
...
function buildCsp(scriptHashes, styleHashes) {
  ...
  const imgSrc = [
    ...
    "https://*.supabase.co",
    "https://dnd-tool.thesnowpost.com",
    "https:", // ✅ Allow all HTTPS images (includes wizards.com, supabase, etc.) TODO: tighten further if possible
  ].join(" ");
